...
dtmlXMLLoaderObject.prototype.getXMLTopNode = function (a, b) {
    ...
    if (!this._retry) return this._retry = !0, b = this.xmlDoc, this.loadXMLString(this.xmlDoc.responseText.replace(/^[\s]+/, ""), !0), this.getXMLTopNode(a, b);
...
dtmlXMLLoaderObject.prototype.doXPath = function (a, b, c, d) {
    ...
    if (_isIE) return b || (b = this.xmlDoc.nodeName ? this.xmlDoc : this.xmlDoc.responseXML), b || dhtmlxError.throwError("LoadXML", "Incorrect XML", [b || this.xmlDoc, this.mainObject]), c != null && b.setProperty("SelectionNamespaces", "xmlns:xsl='" + c + "'"), d == "single" ? b.selectSingleNode(a) : b.selectNodes(a) || [];
        ...
        for (var k = h.iterateNext() ; k;) g[g.length] = k, k = h.iterateNext();
        return g
    }
};
function w() {
...
(function () {
    ...
    function c(a) {
        if (!c.cover) c.cover = document.createElement("DIV"), c.cover.onkeydown = b, c.cover.className = "dhx_modal_cover", document.body.appendChild(c.cover);
        var d = document.body.scrollHeight;
        c.cover.style.display = a ? "inline-block" : "none"
    }
    function d(a, b) {
    ...
    function f(b, c, e) {
        ...
        g += '<div class="dhtmlx_popup_text"><span>' + (b.content ? "" : b.text) + '</span></div><div  class="dhtmlx_popup_controls">';
        c && (g += d(b.ok || "OK", !0));
    ...
    function g(a, d, e) {
        ...
        g.style.top = a.position == "top" ? "-3px" : i + "px";
        g.style.left = h + "px";
        g.onkeydown = b;
        g.focus();
    ...
    var n = null;
    document.attachEvent ? document.attachEvent("onkeydown", b) : document.addEventListener("keydown", b, !0);
    ...
    var m = dhtmlx.message = function (a, b, c, d) {
...
dataProcessor.prototype = {
    ...
    serialize: function (a, b) {
        ...
        if (typeof b != "undefined") return this.serialize_one(a, "");
            ...
            dhtmlx.security_key && c.push("dhx_security=" + dhtmlx.security_key);
            return c.join("&")
        }
    },
...
scheduler.init = function (a, b, c) {
    ...
    (function () {
        ...
        function b(a, c) {
            return a.w == c.w && a.h == c.h
        }
        var c = a();
        dhtmlxEvent(window, "resize", function () {
            var g = a();
            if (!b(c, g)) window.clearTimeout(scheduler._resize_timer), scheduler._resize_timer = window.setTimeout(function () {
...
scheduler.select = function (a) {
    if (this._select_id != a) this.editStop(!1), this.unselect(), this._select_id = a, this.updateEvent(a)
};
...
scheduler._click = {
    dhx_cal_data: function (a) {
        if (scheduler._ignore_next_click) return a.preventDefault && a.preventDefault(), a.cancelBubble = !0, scheduler._ignore_next_click = !1;
...
scheduler._mouse_coords = function (a) {
    ...
    if (this._cols) {
        ...
        if (this._ignores) for (var g = 0; g <= f; g++) this._ignores[g] && f++
    }
    if (this._table_view) {
        if (!this._cols || !this._colsS) return b;
        for (var h = 0, h = 1; h < this._colsS.heights.length; h++) if (this._colsS.heights[h] > b.y) break;
        b.y = Math.ceil((Math.max(0, f) + Math.max(0, h - 1) * 7) * 1440 / this.config.time_step);
        if (scheduler._drag_mode || this._mode == "month") b.y = (Math.max(0, Math.ceil(f) - 1) + Math.max(0, h - 1) * 7) * 1440 / this.config.time_step;
        if (this._drag_mode == "move" && scheduler._ignores_detected && scheduler.config.preserve_length && (b._ignores = !0, !this._drag_event._event_length)) this._drag_event._event_length = this._get_real_event_length(this._drag_event.start_date, this._drag_event.end_date, {
            x_step: 1,
            x_unit: "day"
        });
        b.x = 0
    } else {
        if (!this._cols) return b;
    ...
    if (this._drag_mode) {
        ...
        if (!this._drag_pos || b.force_redraw || this._drag_pos.x != b.x || this._drag_pos.y != b.y) {
            ...
            if (this._drag_mode == "create") {
                ...
                if (!this._drag_start) {
                    var e = this.callEvent("onBeforeEventCreated", [a, this._drag_id]);
                    if (!e) return;
                    this._drag_start = c;
                    return
                }
                ...
                var f = new Date(this._drag_start),
                    g = new Date(d);
                if ((this._mode == "day" || this._mode == "week") && f.getHours() == g.getHours() && f.getMinutes() == g.getMinutes()) g = new Date(this._drag_start + 1E3);
            ...
            if (this._drag_mode == "move") if (c = this._min_date.valueOf() + (b.y * this.config.time_step + b.x * 1440 - (scheduler._move_pos_shift || 0)) * 6E4, !b.custom && this._table_view && (c += this.date.time_part(h.start_date) * 1E3), c = this._correct_shift(c), b._ignores && this.config.preserve_length && this._table_view) {
                if (this.matrix) var k = this.matrix[this._mode];
                k = k || {
                    x_step: 1,
                    x_unit: "day"
                };
                d = c * 1 + this._get_fictional_event_length(c, this._drag_event._event_length, k)
            } else d = h.end_date.valueOf() - (h.start_date.valueOf() - c);
            else {
                c = h.start_date.valueOf();
                d = h.end_date.valueOf();
                if (this._table_view) {
                    var i = this._min_date.valueOf() + b.y * this.config.time_step * 6E4 + (b.custom ? 0 : 864E5);
                    this._mode == "month" && (i = this._correct_shift(i, !1));
                    b.resize_from_start ? c = i : d = i
                } else if (d = this.date.date_part(new Date(h.end_date)).valueOf() + b.y * this.config.time_step * 6E4, this._els.dhx_cal_data[0].style.cursor = "s-resize", this._mode == "week" || this._mode == "day") d = this._correct_shift(d);
            ...
            var l = new Date(d - 1),
                n = new Date(c);
            if (this._table_view || l.getDate() == n.getDate() && l.getHours() < this.config.last_hour || scheduler._allow_dnd) if (h.start_date = n, h.end_date = new Date(d), this.config.update_render) {
        ...
        var o = this._locate_event(a.target || a.srcElement);
...
scheduler._on_mouse_down = function (a, b) {
    if (a.button != 2 && !this.config.readonly && !this._drag_mode) {
        ...
        if (this._drag_mode) {
            var d = this._locate_event(b);
            !this.config["drag_" + this._drag_mode] || !this.callEvent("onBeforeDrag", [d, this._drag_mode, a]) ? this._drag_mode = this._drag_id = 0 : (this._drag_id = d, this._drag_event = scheduler._lame_clone(this.getEvent(this._drag_id) || {}))
        }
...
scheduler.isViewExists = function (a) {
    return !!(scheduler[a + "_view"] || scheduler.date[a + "_start"] && scheduler.templates[a + "_date"] && scheduler.templates[a + "_scale_date"])
};
...
scheduler.setCurrentView = function (a, b) {
    if (this.callEvent("onBeforeViewChange", [this._mode, this._date, b || this._mode, a || this._date])) this.updateView(a, b), this.callEvent("onViewChange", [this._mode, this._date])
};
...
scheduler._reset_scale = function () {
    if (this.templates[this._mode + "_date"]) {
        ...
        for (var m = l - this._ignores_detected, o = 0; o < l; o++) {
            this._ignores[o] ? (this._cols[o] = 0, m++) : (this._cols[o] = Math.floor(e / (m - o)), this._render_x_header(o, f, g, a));
            if (!this._table_view) {
            ...
            e -= this._cols[o];
            f += this._cols[o];
            this._colsS[o] = (this._cols[o - 1] || 0) + (this._colsS[o - 1] || (this._table_view ? 0 : this.xy.scale_width + 2));
        ...
        if (this._table_view) this._reset_month_scale(b, h, k);
            ...
            var s = this._els.dhx_cal_navline[0],
                u = s.offsetHeight + this._els.dhx_cal_header[0].offsetHeight + 1,
...
scheduler._process_ignores = function (a, b, c, d, e) {
    ...
    if (f) for (var g = new Date(a), h = 0; h < b; h++) f(g) && (this._ignores_detected += 1, this._ignores[h] = !0, e && b++), g = scheduler.date.add(g, d, c)
};
scheduler._reset_month_scale = function (a, b, c) {
    ...
    var l = 0;
    this._min_date = c;
    for (var n = "<table cellpadding='0' cellspacing='0'>", m = [], i = 0; i < f; i++) {
        n += "<tr>";
        for (var o = 0; o < 7; o++) {
            n += "<td";
            var p = "";
    ...
    for (i = 0; i < m.length; i++) t = m[i], this.callEvent("onScaleAdd", [this._scales[+t], t]);
    return c
};
...
scheduler._get_real_event_length = function (a, b, c) {
    var d = b - a,
        e = c._start_correction + c._end_correction || 0,
        f = this["ignore_" + this._mode],
        g = 0;
    if (c.render) var g = this._get_date_index(c, a),
        h = this._get_date_index(c, b);
    else h = Math.round(d / 60 / 60 / 1E3 / 24);
    for (; g < h;) {
...
scheduler._get_fictional_event_length = function (a, b, c, d) {
    ...
    if (c._start_correction || c._end_correction) {
        var g = d ? e.getHours() * 60 + e.getMinutes() - (c.first_hour || 0) * 60 : (c.last_hour || 0) * 60 - (e.getHours() * 60 + e.getMinutes()),
            h = (c.last_hour - c.first_hour) * 60,
            k = Math.ceil((b / 6E4 - g) / h);
        b += k * (1440 - h) * 6E4
    }
    var i = new Date(a * 1 + b * f),
        j = this["ignore_" + this._mode],
        l = 0;
    if (c.render) var l = this._get_date_index(c, e),
        n = this._get_date_index(c, i);
    else n = Math.round(b / 60 / 60 / 1E3 / 24);
    for (; l * f <= n * f;) {
        var m = scheduler.date.add(e, c.x_step * f, c.x_unit);
        j && j(e) && (b += (m - e) * f, n += f);
        e = m;
        l += f
    }
...
scheduler.deleteEvent = function (a, b) {
    ...
    if (b || this.callEvent("onBeforeEventDelete", [a, c]) && this.callEvent("onConfirmedBeforeEventDelete", [a, c])) c && (delete this._events[a], this.unselect(a), this.event_updated(c)), this.callEvent("onEventDeleted", [a, c])
};
...
scheduler.render_view_data = function (a, b) {
    ...
    for (var c = 0, d = a.length; c < d; c++) this._recalculate_timed(a[c]);
    if (this.config.multi_day && !this._table_view) {
        for (var e = [], f = [], c = 0; c < a.length; c++) this._is_main_area_event(a[c]) ? e.push(a[c]) : f.push(a[c]);
...
scheduler._render_month_link = function (a) {
    for (var b = this._rendered_location, c = this._lame_clone(a), d = a._sday; d < a._eday; d++) {
        c._sday = d;
        c._eday = d + 1;
        var e = scheduler.date,
            f = scheduler._min_date,
            f = e.add(f, c._sweek, "week"),
            f = e.add(f, c._sday, "day"),
            g = scheduler.getEvents(f, e.add(f, 1, "day")).length,
            h = this._get_event_bar_pos(c),
            k = h.x2 - h.x,
...
scheduler._recalculate_timed = function (a) {
    if (a) {
        ...
        if (b) b._timed = scheduler.isOneDayEvent(b)
    }
};
...
scheduler.render_data = function (a, b) {
    for (var a = this._pre_render_events(a, b), c = 0; c < a.length; c++) if (this._table_view) if (scheduler._mode != "month") this.render_event_bar(a[c]);
    else {
        ...
        if (g.rows) {
            for (var h = 0; h < g.rows.length; h++) {
                ...
                if (e[h] * c > this._colsS.height - 22) {
                    var k = g.rows[h].cells,
                        i = this._colsS.height - 22;
                    this.config.max_month_events * 1 !== this.config.max_month_events || e[h] <= this.config.max_month_events ? i = e[h] * c : (this.config.max_month_events + 1) * c > this._colsS.height - 22 && (i = (this.config.max_month_events + 1) * c);
                    for (var j = 0; j < k.length; j++) k[j].childNodes[1].style.height = i + "px";
            ...
            if (g.parentNode.offsetHeight < g.parentNode.scrollHeight && !g._h_fix && scheduler.xy.scroll_width) {
                for (h = 0; h < g.rows.length; h++) {
                    for (var l = 6; this._ignores[l];) l--;
                    var n = g.rows[h].cells[l].childNodes[0],
                        m = n.offsetWidth - scheduler.xy.scroll_width + "px";
                    n.style.width = m;
            ...
            e[0] = 0;
            if (q != p) f.style.top = parseInt(f.style.top) + 2 + "px", s.style.overflowY = "auto", s.style.width = parseInt(s.style.width) - 2 + "px", u.style.position = "fixed", u.style.top = "", u.style.left = ""
        }
    }
    return a
};
...
scheduler._pre_render_events_line = function (a, b) {
    ...
    for (var e = 0; e < a.length; e++) {
        ...
        if (this._ignores[f._sday]) a.splice(e, 1), e--;
        else {
            c[f._sday] || (c[f._sday] = []);
            if (!b) {
                f._inner = !1;
                for (var j = c[f._sday]; j.length;) {
                    var l = j[j.length - 1],
                        n = this._get_event_mapped_end_date(l);
                    if (n.valueOf() <= f.start_date.valueOf()) j.splice(j.length - 1, 1);
                    else break
                }
                for (var m = !1, o = 0; o < j.length; o++) if (l = j[o], n = this._get_event_mapped_end_date(l), n.valueOf() <= f.start_date.valueOf()) {
                    m = !0;
                    f._sorder = l._sorder;
                    j.splice(o, 1);
                    f._inner = !0;
                ...
                if (!m) if (j.length) if (j.length <= j[j.length - 1]._sorder) {
                    if (j[j.length - 1]._sorder) for (o = 0; o < j.length; o++) {
                        for (var p = !1, q = 0; q < j.length; q++) if (j[q]._sorder == o) {
                            p = !0;
                            break
                        }
                        if (!p) {
                    ...
                    p = j[0]._sorder;
                    for (o = 1; o < j.length; o++) if (j[o]._sorder > p) p = j[o]._sorder;
                    f._sorder = p + 1;
                    f._inner = !1
                } else f._sorder = 0;
                j.push(f);
            ...
            if (k < this.config.first_hour || i >= this.config.last_hour) if (d.push(f), a[e] = f = this._copy_event(f), k < this.config.first_hour && (f.start_date.setHours(this.config.first_hour), f.start_date.setMinutes(0)), i >= this.config.last_hour && (f.end_date.setMinutes(0), f.end_date.setHours(this.config.last_hour)), f.start_date > f.end_date || k == this.config.last_hour) a.splice(e, 1), e--
        }
    }
    if (!b) {
...
scheduler._pre_render_events_table = function (a, b) {
    ...
    for (var c = [], d = [
        ...
        [],
        []
    ], e = this._colsS.heights, f, g = this._cols.length, h = {}, k = 0; k < a.length; k++) {
        var i = a[k],
            j = i.id;
        h[j] || (h[j] = {
            first_chunk: !0,
        ...
        var l = h[j],
            n = f || i.start_date,
            m = i.end_date;
        if (n < this._min_date) l.first_chunk = !1, n = this._min_date;
        if (m > this._max_date) l.last_chunk = !1, m = this._max_date;
        var o = this.locate_holder_day(n, !1, i);
        i._sday = o % g;
        if (!this._ignores[i._sday] || !i._timed) {
            var p = this.locate_holder_day(m, !0, i) || g;
            i._eday = p % g || g;
            i._length = p - o;
            i._sweek = Math.floor((this._correct_shift(n.valueOf(), 1) - this._min_date.valueOf()) / (864E5 * g));
            var q = d[i._sweek],
                r;
            for (r = 0; r < q.length; r++) if (q[r]._eday <= i._sday) break;
            if (!i._sorder || !b) i._sorder = r;
            if (i._sday + i._length <= g) f = null, c.push(i), q[r] = i, e[i._sweek] = q.length - 1, i._first_chunk = l.first_chunk, i._last_chunk = l.last_chunk;
            else {
                ...
                s._sweek = i._sweek;
                s._sorder = i._sorder;
                s.end_date = this.date.add(n, s._length, "day");
                if (s._first_chunk = l.first_chunk) l.first_chunk = !1;
                c.push(s);
                q[r] = s;
                f = s.end_date;
...
scheduler.render_event = function (a) {
    var b = scheduler.xy.menu_width,
        c = this.config.use_select_menu_space ? 0 : b;
    if (!(a._sday < 0)) {
        var d = scheduler.locate_holder(a._sday);
        if (d) {
            var e = a.start_date.getHours() * 60 + a.start_date.getMinutes(),
                f = a.end_date.getHours() * 60 + a.end_date.getMinutes() || scheduler.config.last_hour * 60,
                g = a._count || 1,
                h = a._sorder || 0,
                k = Math.round((e * 6E4 - this.config.first_hour * 36E5) * this.config.hour_size_px / 36E5) % (this.config.hour_size_px * 24),
                i = Math.max(scheduler.xy.min_event_height, (f - e) * this.config.hour_size_px / 60),
                j = Math.floor((d.clientWidth - c) / g),
                l = h * j + 1;
            a._inner || (j *= g - h);
            if (this.config.cascade_event_display) var n = this.config.cascade_event_count,
                m = this.config.cascade_event_margin,
                l = h % n * m,
                o = a._inner ? (g - h - 1) % n * m / 2 : 0,
                j = Math.floor(d.clientWidth - c - l - o);
            ...
            if (this._edit_id == a.id) {
                ...
                if (this._quirks7) q.firstChild.style.height = i - 12 + "px";
            ...
            if (this.xy.menu_width !== 0 && this._select_id == a.id) {
                if (this.config.cascade_event_display && this._drag_mode) p.style.zIndex = 1;
                for (var r = this.config["icons_" + (this._edit_id == a.id ? "edit" : "select")], s = "", u = a.color ? "background-color: " + a.color + ";" : "", t = a.textColor ? "color: " + a.textColor + ";" : "", v = 0; v < r.length; v++) s += "<div class='dhx_menu_icon " + r[v] + "' style='" + u + "" + t + "' title='" + this.locale.labels[r[v]] + "'></div>";
...
scheduler._render_v_bar = function (a, b, c, d, e, f, g, h, k) {
    var i = document.createElement("DIV"),
        j = this.getEvent(a),
        l = k ? "dhx_cal_event dhx_cal_select_menu" : "dhx_cal_event",
        n = scheduler.templates.event_class(j.start_date, j.end_date, j);
    n && (l = l + " " + n);
    var m = j.color ? "background:" + j.color + ";" : "",
        o = j.textColor ? "color:" + j.textColor + ";" : "",
        p = '<div event_id="' + a + '" class="' + l + '" style="position:absolute; top:' + c + "px; left:" + b + "px; width:" + (d - 4) + "px; height:" + e + "px;" + (f || "") + '"></div>';
    i.innerHTML = p;
...
scheduler._get_dnd_order = function (a, b, c) {
    if (!this._drag_event) return a;
    this._drag_event._orig_sorder ? a = this._drag_event._orig_sorder : this._drag_event._orig_sorder = a;
    for (var d = b * a; d + b > c;) a--, d -= b;
    return a
};
scheduler._get_event_bar_pos = function (a) {
    var b = this._colsS[a._sday],
        c = this._colsS[a._eday];
    c == b && (c = this._colsS[a._eday + 1]);
    var d = this.xy.bar_height,
        e = a._sorder;
    if (a.id == this._drag_id) var f = this._colsS.heights[a._sweek + 1] - this._colsS.heights[a._sweek] - 22,
        e = scheduler._get_dnd_order(e, d, f);
    var g = e * d,
        h = this._colsS.heights[a._sweek] + (this._colsS.height ? this.xy.month_scale_height + 2 : 2) + g;
    return {
        x: b,
        x2: c,
        y: h
...
scheduler.render_event_bar = function (a) {
    ...
    if (f) {
        ...
        var i = a.color ? "background:" + a.color + ";" : "",
            ...
            l = '<div event_id="' + a.id + '" class="' + h + '" style="position:absolute; top:' + d + "px; left:" + e + "px; width:" + (f - e - 15) + "px;" + j + "" + i + "" + (a._text_style || "") + '">',
...
scheduler.showEvent = function (a, b) {
    ...
    if (c && (!this.checkEvent("onBeforeEventDisplay") || this.callEvent("onBeforeEventDisplay", [c, b]))) {
        ...
        var f = c.color,
            g = c.textColor;
        if (scheduler.config.highlight_displayed_event) c.color = scheduler.config.displayed_event_color, c.textColor = scheduler.config.displayed_event_text_color;
        scheduler.setCurrentView(new Date(c.start_date), b);
        c.color = f;
        c.textColor = g;
        scheduler.config.scroll_hour = d;
        scheduler.config.preserve_scroll = e;
        if (scheduler.matrix && scheduler.matrix[b]) scheduler._els.dhx_cal_data[0].scrollTop = getAbsoluteTop(scheduler.getRenderedEvent(c.id)) - getAbsoluteTop(scheduler._els.dhx_cal_data[0]) - 20;
...
scheduler.json.parse = function (a) {
    if (typeof a == "string") scheduler._temp = eval("(" + a + ")"), a = scheduler._temp ? scheduler._temp.data || scheduler._temp.d || scheduler._temp : [];
        ...
        if (f) {
            ...
            for (var g = 0; g < e.length; g++) {
                var h = e[g],
                    ...
                    i;
                for (i in h) if (h.hasOwnProperty(i) && !(i == "value" || i == "label")) k[i] = h[i];
                f.push(k)
            }
        }
    }
    ...
    for (var e = a.doXPath("//coll_options"), f = 0; f < e.length; f++) {
        ...
        if (h) {
            ...
            for (var k = a.doXPath(".//item", e[f]), i = 0; i < k.length; i++) {
                for (var j = k[i], l = j.attributes, n = {
                    key: k[i].getAttribute("value"),
                    label: k[i].getAttribute("label")
                }, m = 0; m < l.length; m++) {
                    var o = l[m];
                    if (!(o.nodeName == "value" || o.nodeName == "label")) n[o.nodeName] = o.nodeValue
                }
                h.push(n)
            }
        }
...
scheduler.attachEvent("onXLE", function () {
    ...
    if (a && typeof a == "object") this._obj.removeChild(a), this.config.show_loading = !0
});
scheduler.ical = {
    parse: function (a) {
...
scheduler.form_blocks = {
    ...
    select: {
        ...
        set_value: function (a, b, c, d) {
            var e = a.firstChild;
            if (!e._dhx_onchange && d.onchange) e.onchange = d.onchange, e._dhx_onchange = !0;
    ...
    time: {
        render: function (a) {
            if (!a.time_format) a.time_format = ["%H:%i", "%d", "%m", "%Y"];
            a._time_format_order = {};
            var b = a.time_format,
                c = scheduler.config,
                d = this.date.date_part(scheduler._currentDate()),
                e = 1440,
                f = 0;
            ...
            for (var g = "", h = 0; h < b.length; h++) {
                ...
                switch (k) {
                    case "%Y":
                        a._time_format_order[3] = h;
                        g += "<select>";
                        for (var i = d.getFullYear() - 5, j = 0; j < 10; j++) g += "<option value='" + (i + j) + "'>" + (i + j) + "</option>";
                        g += "</select> ";
                        break;
                    case "%m":
                        a._time_format_order[2] = h;
                        g += "<select>";
                        for (j = 0; j < 12; j++) g += "<option value='" + j + "'>" + this.locale.date.month_full[j] + "</option>";
                        g += "</select>";
                        break;
                    case "%d":
                        a._time_format_order[1] = h;
                        g += "<select>";
                        for (j = 1; j < 32; j++) g += "<option value='" + j + "'>" + j + "</option>";
                        g += "</select>";
                        break;
                    case "%H:%i":
                        ...
                        for (a._time_values = []; j < e;) {
                            ...
                            var m = d.getDate() != l ? 1 : 0,
                                j = m * 1440 + d.getHours() * 60 + d.getMinutes()
                        }
                        g += "</select>"
                }
            }
        ...
        set_value: function (a, b, c, d) {
            function e(a, b, c) {
                for (var e = d._time_values, f = c.getHours() * 60 + c.getMinutes(), g = f, i = !1, j = 0; j < e.length; j++) {
                    var k = e[j];
                    if (k === f) {
                        i = !0;
                        break
                ...
                a[b + h[0]].value = i ? f : g;
                if (!i && !g) a[b + h[0]].selectedIndex = -1;
            ...
            if (f.full_day) {
                ...
                i.onclick = function () {
                    if (i.checked) {
                        ...
                        var f = scheduler.date.date_part(b.start_date),
                            j = scheduler.date.date_part(b.end_date);
                        if (+j == +f || +j >= +f && (c.end_date.getHours() != 0 || c.end_date.getMinutes() != 0)) j = scheduler.date.add(j, 1, "day")
                    }
                    g[h[0]].disabled = i.checked;
                    g[h[0] + g.length / 2].disabled = i.checked;
            ...
            if (f.auto_end_date && f.event_duration) for (var j = function () {
                    var a = new Date(g[h[3]].value, g[h[2]].value, g[h[1]].value, 0, g[h[0]].value),
                        b = new Date(a.getTime() + scheduler.config.event_duration * 6E4);
                    e(g, 4, b)
            }, l = 0; l < 4; l++) g[l].onchange = j;
            e(g, 0, c.start_date);
            e(g, 4, c.end_date)
        },
        get_value: function (a, b, c) {
...
scheduler._init_lightbox_events = function () {
    ...
    this.getLightbox().onkeydown = function (a) {
...
scheduler.getLightbox = function () {
    if (!this._lightbox) {
        ...
        for (var b = this._lightbox_template, c = this.config.buttons_left, d = 0; d < c.length; d++) b += "<div class='dhx_btn_set dhx_left_btn_set " + c[d] + "_set'><div dhx_button='1' class='" + c[d] + "'></div><div>" + scheduler.locale.labels[c[d]] + "</div></div>";
...
scheduler._touch_events = function (a, b, c) {
    function d(a, b, c) {
        if (a && b) {
            var d = Math.abs(a.pageY - b.pageY),
                e = Math.abs(a.pageX - b.pageX);
            if (e > c && (!d || e / d > 3)) a.pageX > b.pageX ? scheduler._click.dhx_cal_next_button() : scheduler._click.dhx_cal_prev_button()
        }
    }
    function e(a) {
        scheduler._hide_global_tip();
        if (i) scheduler._on_mouse_up(b(a || event)), scheduler._temp_touch_block = !1;
    ...
    dhtmlxEvent(document.body, a[0], function (a) {
        if (!c(a)) {
            if (i) return scheduler._on_mouse_move(b(a)), scheduler._update_global_tip(), a.preventDefault && a.preventDefault(), a.cancelBubble = !0, !1;
            h && f && d(h, b(a), 0);
            h = b(a);
            if (l) if (h) {
                if (g.target != h.target || Math.abs(g.pageX - h.pageX) > 5 || Math.abs(g.pageY - h.pageY) > 5) j = !0, clearTimeout(k)
            } else j = !0
        }
    });
    ...
    dhtmlxEvent(this._els.dhx_cal_data[0], "contextmenu", function (a) {
        if (l) return a && a.preventDefault && a.preventDefault(), (a || event).cancelBubble = !0, !1
    });
    dhtmlxEvent(this._els.dhx_cal_data[0], a[1], function (a) {
        if (!c(a)) {
            ...
            if (d) {
                var e = new Date;
                if (!j && !i && e - n < 250) return scheduler._click.dhx_cal_data(d), window.setTimeout(function () {
                ...
                !j && !i && scheduler.config.touch_drag && (k = setTimeout(function () {
                    i = !0;
                    var a = g.target;
                    if (a && a.className && a.className.indexOf("dhx_body") != -1) a = a.previousSibling;
                    ...
                    if (scheduler._drag_mode && scheduler._drag_mode != "create") {
                        ...
                        if (b >= 0) {
                            ...
                            scheduler._move_pos_shift = c * Math.round((d.pageY - b) * 60 / (scheduler.config.hour_size_px * c))
                        }
                    }
    ...
    dhtmlxEvent(this._els.dhx_cal_data[0], a[2], function (a) {
        if (!c(a)) {
            i || d(g, h, 200);
            if (i) scheduler._ignore_next_click = !0;
            e(a);
            if (scheduler._block_next_stop) return scheduler._block_next_stop = !1, a.preventDefault && a.preventDefault(), a.cancelBubble = !0, !1
        }
    });
...
scheduler._hide_global_tip = function () {
    ...
    if (a && a.parentNode) a.parentNode.removeChild(a), scheduler._global_tip = 0
};
scheduler._dp_init = function (a) {
    ...
    this.attachEvent("onEventAdded", function (b) {
        !this._loading && this._validId(b) && a.setUpdated(b, !0, "inserted")
    });
    ...
    this.attachEvent("onEventChanged", function (b) {
        !this._loading && this._validId(b) && a.setUpdated(b, !0, "updated")
    });
...
scheduler._configure = function (a, b, c) {
    for (var d in b) typeof a[d] == "undefined" && (a[d] = b[d][c])
};
scheduler._skin_init = function () {
    if (!scheduler.skin) for (var a = document.getElementsByTagName("link"), b = 0; b < a.length; b++) {
    ...
    if (!d) {
        ...
        scheduler.attachEvent("onTemplatesReady", function () {
            var a = scheduler.date.date_to_str("%d"),
                b = scheduler.templates.month_day;
            scheduler.templates.month_day = function (c) {
                if (this._mode == "month") {
                    var d = a(c);
                    c.getDate() == 1 && (d = scheduler.locale.date.month_full[c.getMonth()] + " " + d); +c == +scheduler.date.date_part(new Date) && (d = scheduler.locale.labels.dhx_cal_today_button + " " + d);
                    return d
                } else return b.call(this, c)
            };
            if (scheduler.config.fix_tab_position) for (var c = scheduler._els.dhx_cal_navline[0].getElementsByTagName("div"), d = [], e = 211, j = 0; j < c.length; j++) {
                var l = c[j],
                    n = l.getAttribute("name");
                if (n) switch (l.style.right = "auto", n) {
                    case "day_tab":
                        l.style.left = "14px";
        ...
        if (typeof c === "string") if (b[c]) return b[c].apply(this, []);
            ...
            this.each(function () {
                if (this && this.getAttribute && !this.getAttribute("dhxscheduler")) {
                    for (var a in c) a != "data" && (scheduler.config[a] = c[a]);
